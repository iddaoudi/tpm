...
#pragma omp single
  {
    for (k = 0; k < matrix_size; k++) {
      ...
      if (TPM_TRACE || TPM_TRACE_NO_OMPT) tpm_upstream_set_task_name(name_with_id_char);
#pragma omp task firstprivate(k) shared(M)                                     \
    depend(inout                                                               \
           : M [k * matrix_size + k:tile_size * tile_size])
      {
          ...
          if (TPM_TRACE || TPM_TRACE_NO_OMPT) getcpu(&cpu, &node);
          if (TPM_TRACE || TPM_TRACE_NO_OMPT) tpm_upstream_set_task_cpu_node(cpu, node,
          name_with_id_char);
          if (TPM_TRACE || TPM_TRACE_NO_OMPT) gettimeofday(&start, NULL); 
          lu0(M[k * matrix_size + k], tile_size);
          if (TPM_TRACE || TPM_TRACE_NO_OMPT) gettimeofday(&end, NULL);
          if (TPM_TRACE || TPM_TRACE_NO_OMPT) tpm_upstream_get_task_time(start, end, name_with_id_char);
      }
    ...
    depend(in                                                                  \
        ...
        depend(inout                                                           \
               : M [k * matrix_size + j:tile_size * tile_size])
          {
            ...
            if (TPM_TRACE || TPM_TRACE_NO_OMPT) getcpu(&cpu, &node);
            if (TPM_TRACE || TPM_TRACE_NO_OMPT) tpm_upstream_set_task_cpu_node(cpu, node, name_with_id_char);
            if (TPM_TRACE || TPM_TRACE_NO_OMPT) gettimeofday(&start, NULL); 
