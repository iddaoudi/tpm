...
//void tpm_sparse_generator (tpm_desc spA) {
void tpm_sparse_generator (double *M[], int matrix_size, int tile_size) {
	int init_val = 1325;
	int zero_element = 0;
	int i = 0, j = 0, m = 0, n = 0;
	...
	for (m = 0; m < matrix_size; m++)
	{
		for (n = 0; n < matrix_size; n++)
		{
			...
			zero_element = 0;
			if ((m < n) && (m%3 != 0)) zero_element = 1;
			if ((m > n) && (n%3 != 0)) zero_element = 1;
			if (m%2 == 1) zero_element = 1;
			if (n%2 == 1) zero_element = 1;
			if (m == n) zero_element = 0;
			if (m == n-1) zero_element = 0;
			if (m-1 == n) zero_element = 0;
			/* allocating matrix */
			if (zero_element == 0)
			{
				M[m * matrix_size + n] = (double *) malloc(tile_size * tile_size * sizeof(double));
...
void tpm_sparse_allocate (double*** M, int matrix_size, int tile_size){
